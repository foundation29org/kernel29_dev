...
if __name__ == "__main__":
    ...
    differential_diagnosis_model = 'c3opus'  # Model name used to get differential diagnoses
...
# Get golden diagnoses for those cases
    ...
    text_dict = dif_diagnosis_dict2plain_text_dict_with_real_diagnosis(dif_diagnosis_dict,
        case_diagnoses,
        separator_string = "The are the differential diagnoses to evaluate against the golden diagnosis:\n\n")
    if verbose:
    ...
    semantic_categories = set(["Exact synonym", "Broad synonym", "Exact group of diseases", "Broad group of diseases", "Related disease group", "Not related disease"])
    ...
    for result in semantic_judge_results:
        print(result)
        input("Press Enter to continue...")
    add_semantic_results_to_db(semantic_judge_results, session)
    end_time = time.time()
