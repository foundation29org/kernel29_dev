...
def load_data(csv_path, verbose=False):
    ...
    try:
        ...
        if verbose:
            ...
            # print(f"OUTPUT Data Head:\n{df.head().to_string()}") # Optionally print head
        return df
...
def calculate_scores_per_group(df_filtered, d_max, verbose=False):
    ...
    if not all(col in df_filtered.columns for col in required_cols):
        ...
        sys.exit(1)
    # Group by the unique ID for each prediction list
...
def save_results_csv(results_df, output_path, combo_col_name, verbose=False):
    ...
    if combo_col_name not in results_df.columns:
        ...
        desired_order.remove(combo_col_name)
...
# --- Plotting Functions ---
# (Plots now use combo_col passed as argument)
...
def plot_original_violin(results_df, combo_col, output_dir, filename="plot_01_original_violin.png", verbose=False):
    ...
    if results_df.empty or combo_col not in results_df.columns:
        print(f"WARNING: No data or column '{combo_col}' found to plot."); return
    n_combos = results_df[combo_col].nunique()
    figsize_width = max(12, n_combos * 1.5)
    ...
    plt.tight_layout()
    try: plt.savefig(output_path, dpi=150)
...
def plot_score_by_golden_severity(results_df, combo_col, output_dir, filename="plot_04_score_by_golden_severity.png", verbose=False):
    ...
    if results_df.empty or combo_col not in results_df.columns or 'golden_diagnosis_severity' not in results_df.columns:
         print(f"WARNING: No data or required columns found to plot."); return
    n_combos = results_df[combo_col].nunique()
    figsize_width = 14 + max(0, (n_combos - 4) * 0.5)
...
def plot_confusion_matrices(mapped_df, severity_map, output_dir, prefix="plot_06_confusion_matrix_", verbose=False):
    ...
    for model, prompt in model_prompt_combos:
        safe_prompt_name = prompt.replace(' ', '_').replace('/', '_').replace(':','_') # Make filename safer
...
if __name__ == "__main__":
    ...
    OUTPUT_CSV = 'severity_scores_final_output_by_combo.csv'
    OUTPUT_PLOT_DIR = 'analysis_plots_by_combo'
    ...
    # Only plot distance if weighted_df has the combo column
    if COMBINED_ID_COLUMN in weighted_df.columns:
        ...
        if VERBOSE_MODE: print(f"INFO: Skipping distance vs rank plot as '{COMBINED_ID_COLUMN}' is missing in weighted_df.")
    ...
    if COMBINED_ID_COLUMN in final_results.columns:
        ...
        if len(combos) >= 2:
            ...
            for combo1, combo2 in itertools.combinations(combos, 2):
                ...
                if not scores1.empty and not scores2.empty and len(scores1)>1 and len(scores2)>1: # Test needs >1 value per group
                    try:
                ...
                # else: # Optional: Report pairs with insufficient data
                   # if VERBOSE_MODE: print(f"  - {combo1} vs {combo2}: WARNING - Not enough data for comparison.")
            ...
            if significant_pairs:
                ...
                print("INFO: No statistically significant differences found between any pair of combinations (at alpha=0.05).")
        else:
